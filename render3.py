...
def render_community(template_path, data):
    ...
    try:
        data['state']['lastchange'] = dateutil.parser.parse(
            data['state']['lastchange'], tzinfos=tzoffset(None, 18000))
    except:
        pass
...
def validate_community(specs, instance):
    if instance['api'] not in specs.keys():
        print(
            "[+] Invalid or unknown API version {}: {}".format(instance['api'], instance['url']))
        return
    ...
    if errors:
        status = "invalid"
        status_text = "Invalid"
        result = "<ul>{}</ul".format(''.join("<li>Error in {}: {}</li>".format(
            "->".join(str(path) for path in err.path), err.message) for err in errors))
...
def main():
    ...
    ghub_repo = requests.get(
        "https://github.com/freifunk/api.freifunk.net/archive/refs/heads/master.zip")
...
if __name__ == '__main__':
    try:
        ...
        print("\n[+] Encountered Errors")
        removeTempFile("api_freifunk.zip",
